...
Use `mcp__glm__glm_agent` for repository tasks and pass an absolute `workdir`. Use `dry_run: true` when the proposed patch needs review. Text-only work also goes through `mcp__glm__glm_agent` (no file edits needed; `mcp__glm__glm_delegate` is opt-in via GLM_DELEGATE=on). If uncertain, call `mcp__glm__glm_recommend` first.
